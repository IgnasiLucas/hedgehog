...
def get_allele_depth(record, pop, ref, alt, sampledict):
	...
	for call in record.samples:
		...
		allele_depth = {'C': 0, 'A': 0, 'T': 0, 'G': 0}
		try:
			allele_depth[ref] = call["AD"][0]
			allele_depth[alt] = call["AD"][1]
		# Samples without data (./.:.:.:.:.) give a TypeError
		except TypeError:
			allele_depth[ref] = 0
			allele_depth[alt] = 0
...
def get_error_rates(record, pop, sampledict):
	"""
	The arithmetic mean of Phred quality scores among reads corresponds to the
	geometric mean of their probabilities of error.

	Input:
		record: pyvcf reader object.
		pop: population ID that is key in sampledict.
		sampledict: dict(list) {population: [inds]}.
	"""
	ref_obs = record.INFO["RO"]
	alt_obs = record.INFO["AO"][0]
	sum_ref_qual = record.INFO["QR"]
	sum_alt_qual = record.INFO["QA"][0]

	mean_qual = (sum_ref_qual + sum_alt_qual) / (ref_obs + alt_obs)
	geom_error_prob = 10 ** (-mean_qual / 10)
	return geom_error_prob
...
def write_output(record, sampledict, ref, alt, locus, prefix, admix, p1, p2):
	...
	# Get error rates
	admix_errors = get_error_rates(record, "Admixed", sampledict)

	admix.write("{} {}\npop_0\n".format(locus, admix_errors))
	p1.write("{}\n".format(locus))
	p2.write("{}\n".format(locus))
