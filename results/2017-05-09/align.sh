...
if [ $1 == bowtie1 ]; then
   if [ $4 == sensitive ]; then
      if [ ! -e bowtie1/trimmed.sensitive/output.bam ]; then
         ...
         $2 -n 3 -l 60 -e 80 --best -p 20 --nomaqround -t -S --sam-nohead bowtie1/erinaceus $3 | samtools view -Sb - > $5/output.bam
      fi
      ...
      if [ $4 == conservative ]; then
         if [ ! -e bowtie1/trimmed.conservative/output.bam ]; then
            ...
            # ambiguous mappings.
            $2 -n 1 -l 50 -e 60 -m 1 -p 20 --nomaqround -t -S --sam-nohead bowtie1/erinaceus $3 | samtools view -Sb - > $5/output.bam
         fi
   ...
   if [ $1 == bowtie2 ]; then
      if [ $4 == sensitive ]; then
         if [ ! -e bowtie2/trimmed.sensitive/output.bam ]; then
            ...
            $2 -N 1 -L 20 -D 25 -R 2 -i S,1,1 -t --no-unal --no-hd -p 20 -x bowtie2/erinaceus -U $3 | samtools view -Sb - > $5/output.bam
         fi
         ...
         if [ $4 == conservative ]; then
            if [ ! -e bowtie2/trimmed.conservative/output.bam ]; then
               ...
               $2 -N 0 -L 22 -D 10 -R 2 -i S,1,1.15 -t --no-unal --no-hd -p 20 -x bowtie2/erinaceus -U $3 | samtools view -Sb - > $5/output.bam
            fi
