...
#  -v OUTGROUP=<str>    Name of the outgroup population. Default: "Hemiechinus".
...
BEGIN{
   ...
   ALT_OUTGROUP_FREQ = altfreq(BPF[2], BPF[3], BINARY_MASK[OUTGROUP])
   if (ALT_OUTGROUP_FREQ + 0.0 == 0.0) {
      DERIVED_FOCAL_FREQ = ALT_FOCAL_FREQ
      DERIVED_SISTER_FREQ = ALT_SISTER_FREQ
   } else {
      if (ALT_OUTGROUP_FREQ + 0.0 == 1.0) {
         DERIVED_FOCAL_FREQ = 1.0 - ALT_FOCAL_FREQ
         DERIVED_SISTER_FREQ = 1.0 - ALT_SISTER_FREQ
      } else {
         # I ignore sites where the outgroup is heterozygous, for I don't know what the ancestral allele is.
         next
      }
   }
   # A site fully counts as divergent if either allele is fixed in the focal population
   # and the other allele, in the other population. In addition, if the focal population
   # is monomorphic and the sister population is polymorphic, the site contributes partially
   # to divergence, to the extent of the other allele's frequency in the sister population.
   if (DERIVED_FOCAL_FREQ + 0.0 == 0.0) {
      DIVERGENCE[EFFECT] += DERIVED_SISTER_FREQ
   } else {
      if (DERIVED_FOCAL_FREQ + 0.0 < 1.0) {
         POLYMORPHISM[sprintf("%.1f", DERIVED_FOCAL_FREQ), EFFECT] += 1
      } else {
